...
@Component({
  selector: 'qrcode-all',
  templateUrl: './qrcode.all.html',
  styleUrls: ['./qrcode.all.css']
})
export class QrcodeAllComponent extends AbstractComponent{
  ...
  createQrCode(){
    ...
    if(this.qrCode.content.length > 1000){
      this.wzlNgZorroAntdMessage.error("二维码内容不能大于1000字符");
      return;
    }

    let condition = this.qrCode;
    this.commonService.doHttpPost(urls.getQrCodeUrl,condition).then(rst =>{
      if (rst) {
